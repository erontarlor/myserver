...
installNextCloud()
{
  ...
  call "echo \"      - NEXTCLOUD_OVERWRITE_HOST=${certificateDomain[0]}\" >> $compose"
  call "echo \"      - NEXTCLOUD_OVERWRITE_PROTOCOL=https\" >> $compose"
  call "echo \"      - NEXTCLOUD_OVERWRITE_WEBROOT=/\" >> $compose"
  call "echo \"      - NEXTCLOUD_OVERWRITE_CLI_URL=https://${certificateDomain[0]}\" >> $compose"
  call "echo \"      - NEXTCLOUD_HTACCESS_REWRITE_BASE=/\" >> $compose"
  call "echo \"      - NEXTCLOUD_DB_TYPE=mysql\" >> $compose"
  call "echo \"      - NEXTCLOUD_DB_NAME=nextcloud\" >> $compose"
  call "echo \"      - NEXTCLOUD_DB_USERNAME=nextcloud\" >> $compose"
  call "echo \"      - NEXTCLOUD_DB_PASSWORD=nextcloud\" >> $compose"
  call "echo \"      - NEXTCLOUD_DB_HOST=db\" >> $compose"
  call "echo \"      - NEXTCLOUD_ADMIN_USERNAME=admin\" >> $compose"
  call "echo \"      - NEXTCLOUD_ADMIN_PASSWORD=$nextCloudPassword\" >> $compose"
  call "echo \"      - NEXTCLOUD_MYSQL_UTF8MB4=true\" >> $compose"
  call "echo \"      - REDIS_ENABLED=true\" >> $compose"
...
#  call "docker-compose exec nextcloud mysql -h db -pnextcloud -P 3306 -u nextcloud -D nextcloud -e \"update oc_users set password='$passwordSha1' where uid='admin';\""
  ...
  call "docker-compose exec nextcloud php occ market:install announcementcenter"
  call "docker-compose exec nextcloud php occ market:install audioplayer"
  call "docker-compose exec nextcloud php occ market:install brute_force_protection"
  call "docker-compose exec nextcloud php occ market:install calendar"
  call "docker-compose exec nextcloud php occ market:install cms_pico"
  call "docker-compose exec nextcloud php occ market:install contacts"
  call "docker-compose exec nextcloud php occ market:install drawio"
  call "docker-compose exec nextcloud php occ market:install files_pdfviewer"
  call "docker-compose exec nextcloud php occ market:install files_reader"
  call "docker-compose exec nextcloud php occ market:install files_texteditor"
  call "docker-compose exec nextcloud php occ market:install files_textviewer"
  call "docker-compose exec nextcloud php occ market:install gallery"
  call "docker-compose exec nextcloud php occ market:install notes"
  call "docker-compose exec nextcloud php occ market:install password_policy"
  call "docker-compose exec nextcloud php occ market:install polls"
  call "docker-compose exec nextcloud php occ market:install tasks"
  call "docker-compose exec nextcloud php occ market:install wallpaper"
  echo "Adding users..."
  declare count=$userCount
  while [ "$count" -gt 0 ]
  do
    let count=count-1
    call "docker-compose exec -e OC_PASS=\"${userPassword[$count]}\" nextcloud php occ user:add --password-from-env --display-name=\"${userFullName[$count]}\" --group=\"users\" --email=${userEMail[$count]} ${userName[$count]}"
  done
