...
#  call "docker-compose exec nextcloud mysql -h db -pnextcloud -P 3306 -u nextcloud -D nextcloud -e \"update oc_users set password='$passwordSha1' where uid='admin';\""
  ...
  call "docker-compose exec -u www-data nextcloud php occ app:install announcementcenter"
  call "docker-compose exec -u www-data nextcloud php occ app:install audioplayer"
#  call "docker-compose exec -u www-data nextcloud php occ app:install brute_force_protection"
  call "docker-compose exec -u www-data nextcloud php occ app:install calendar"
  call "docker-compose exec -u www-data nextcloud php occ app:install cms_pico"
  call "docker-compose exec -u www-data nextcloud php occ app:install contacts"
  call "docker-compose exec -u www-data nextcloud php occ app:install drawio"
  call "docker-compose exec -u www-data nextcloud php occ app:install files_pdfviewer"
#  call "docker-compose exec -u www-data nextcloud php occ app:install files_reader"
#  call "docker-compose exec -u www-data nextcloud php occ app:install files_texteditor"
#  call "docker-compose exec -u www-data nextcloud php occ app:install files_textviewer"
#  call "docker-compose exec -u www-data nextcloud php occ app:install gallery"
  call "docker-compose exec -u www-data nextcloud php occ app:install notes"
  call "docker-compose exec -u www-data nextcloud php occ app:install password_policy"
  call "docker-compose exec -u www-data nextcloud php occ app:install polls"
  call "docker-compose exec -u www-data nextcloud php occ app:install tasks"
#  call "docker-compose exec -u www-data nextcloud php occ app:install wallpaper"
  echo "Adding users..."
  declare count=$userCount
  while [ "$count" -gt 0 ]
  do
    let count=count-1
    call "docker-compose exec -u www-data -e OC_PASS=\"${userPassword[$count]}\" nextcloud php occ user:add --password-from-env --display-name=\"${userFullName[$count]}\" --group=\"users\" --email=${userEMail[$count]} ${userName[$count]}"
  done
